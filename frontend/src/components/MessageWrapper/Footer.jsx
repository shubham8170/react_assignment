...
export const Footer = ({ receiver }) => {
  ...
  const createRoomWithUsers = (senderId, receiverId) => {
  ...
  useEffect(() => {
    ...
    // Create or join the chat room based on user IDs
    const userid = userId?.replace(/"/g, '');
    const roomName = createRoomWithUsers(userid, receiverId);
    ...
    getChats(receiverId).then((chats) => {
      setMessages(chats);
    });
    ...
    newSocket.on('message', (chat) => {
      ...
      if(message?.senderName?.replace(/["/]/g, '')
        !==userId.replace(/["/]/g, '')){
